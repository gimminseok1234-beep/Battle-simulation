...
export class Unit {
    ...
    draw(ctx, isOutlineEnabled, outlineWidth) {
        ...
        if (!use3D) {
            ...
            // Simple limb rig: body capsule, head, arms, legs
            const bodyColor = (()=>{switch(this.team){case TEAM.A:return COLORS.TEAM_A;case TEAM.B:return COLORS.TEAM_B;case TEAM.C:return COLORS.TEAM_C;default:return COLORS.TEAM_D;}})();
            const px = this.pixelX, py = this.pixelY;
            const facing = this.facingAngle;
            const armSwing = Math.sin(this.gameManager.animationFrameCounter*0.15 + (this.attackCooldown>0?2:0)) * GRID_SIZE*0.08;
            const legSwing = Math.sin(this.gameManager.animationFrameCounter*0.18) * GRID_SIZE*0.08;
            const bodyWidth = GRID_SIZE*0.7, bodyHeight = GRID_SIZE*1.0;

            // Shadow ellipse
            ctx.fillStyle = 'rgba(0,0,0,0.25)';
            ctx.beginPath(); ctx.ellipse(px, py + GRID_SIZE*0.5, GRID_SIZE*0.5, GRID_SIZE*0.2, 0, 0, Math.PI*2); ctx.fill();
            ...
            roundRect(ctx, -bodyWidth*0.25, GRID_SIZE*0.25 + legSwing, bodyWidth*0.2, GRID_SIZE*0.6, 4);
            ctx.fill();
            roundRect(ctx, bodyWidth*0.05, GRID_SIZE*0.25 - legSwing, bodyWidth*0.2, GRID_SIZE*0.6, 4);
            ctx.fill();

            // Body
            roundRect(ctx, -bodyWidth*0.35, -bodyHeight*0.3, bodyWidth*0.7, bodyHeight, 10);
            ctx.fill();

            // Arms
            roundRect(ctx, -bodyWidth*0.55, -GRID_SIZE*0.05 + armSwing, bodyWidth*0.2, GRID_SIZE*0.6, 6);
            ctx.fill();
            roundRect(ctx, bodyWidth*0.35, -GRID_SIZE*0.05 - armSwing, bodyWidth*0.2, GRID_SIZE*0.6, 6);
            ctx.fill();

            // Head
            ctx.fillStyle = '#f5f5f5';
            ctx.beginPath(); ctx.arc(0, -bodyHeight*0.5, GRID_SIZE*0.45, 0, Math.PI*2); ctx.fill();
            ctx.fillStyle = bodyColor;
            ctx.beginPath(); ctx.arc(0, -bodyHeight*0.5, GRID_SIZE*0.5, 0, Math.PI*2); ctx.lineWidth = 8; ctx.strokeStyle = bodyColor; ctx.stroke();
            // Eyes
            ctx.fillStyle = '#0f172a';
            ctx.beginPath(); ctx.arc(-GRID_SIZE*0.15, -bodyHeight*0.5, GRID_SIZE*0.08, 0, Math.PI*2); ctx.fill();
            ctx.beginPath(); ctx.arc(GRID_SIZE*0.15, -bodyHeight*0.5, GRID_SIZE*0.08, 0, Math.PI*2); ctx.fill();
