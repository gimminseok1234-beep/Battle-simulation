...
export class Unit {
    ...
    draw(ctx, isOutlineEnabled, outlineWidth) {
        ...
        // Eyes: dynamic expressions based on state
        {
            ...
            const isSpecial = (
                ...
                (this.weapon?.type === 'magic_dagger' && this.isAimingMagicDagger) ||
            ...
            if (isDead) {
                ...
                ctx.lineWidth = Math.max(1.5, eyeRadius * 0.9);
                // Left X eye
                ctx.beginPath();
                ctx.moveTo(-eyeOffsetX - eyeRadius, eyeOffsetY - eyeRadius);
                ctx.lineTo(-eyeOffsetX + eyeRadius, eyeOffsetY + eyeRadius);
                ctx.moveTo(-eyeOffsetX + eyeRadius, eyeOffsetY - eyeRadius);
                ctx.lineTo(-eyeOffsetX - eyeRadius, eyeOffsetY + eyeRadius);
                ctx.stroke();
                // Right X eye
                ...
                ctx.beginPath(); ctx.arc(-eyeOffsetX, eyeOffsetY, eyeRadius, 0, Math.PI * 2); ctx.fill();
                ctx.beginPath(); ctx.arc( eyeOffsetX, eyeOffsetY, eyeRadius, 0, Math.PI * 2); ctx.fill();

                // Pupil target: move, fight, idle
                let pupilTargetX = 0, pupilTargetY = 0;
                if (isFighting && this.target) {
                    pupilTargetX = this.target.pixelX - this.pixelX;
                    pupilTargetY = this.target.pixelY - this.pixelY;
                } else if (isMoving && this.moveTarget) {
                    pupilTargetX = this.moveTarget.x - this.pixelX;
                    pupilTargetY = this.moveTarget.y - this.pixelY;
                } else {
                    const t = this.gameManager.animationFrameCounter * 0.07 + (this.pixelX + this.pixelY) * 0.001;
                    pupilTargetX = Math.cos(t);
                    pupilTargetY = Math.sin(t * 1.3);
                }

                const angle = Math.atan2(pupilTargetY, pupilTargetX);
                const pupilMaxOffset = eyeRadius * 0.45;
                const pupilX = Math.cos(angle) * pupilMaxOffset;
                const pupilY = Math.sin(angle) * pupilMaxOffset;

                // Pupils / special slit and glow
                if (isSpecial) {
                    const glow = this.gameManager.animationFrameCounter;
                    const hue = (glow * 4) % 360;
                    // glow behind pupils
                    ctx.fillStyle = `hsla(${hue}, 90%, 60%, 0.55)`;
                    ctx.beginPath(); ctx.arc(-eyeOffsetX + pupilX, eyeOffsetY + pupilY, eyeRadius * 0.9, 0, Math.PI * 2); ctx.fill();
                    ctx.beginPath(); ctx.arc( eyeOffsetX + pupilX, eyeOffsetY + pupilY, eyeRadius * 0.9, 0, Math.PI * 2); ctx.fill();
                    // slit pupils
                    ctx.strokeStyle = '#0b1020';
                    ctx.lineWidth = Math.max(1.2, eyeRadius * 0.65);
                    ctx.beginPath();
                    ctx.moveTo(-eyeOffsetX + pupilX, eyeOffsetY - eyeRadius * 0.35 + pupilY);
                    ctx.lineTo(-eyeOffsetX + pupilX, eyeOffsetY + eyeRadius * 0.35 + pupilY);
                    ctx.stroke();
                    ctx.beginPath();
                    ctx.moveTo( eyeOffsetX + pupilX, eyeOffsetY - eyeRadius * 0.35 + pupilY);
                    ctx.lineTo( eyeOffsetX + pupilX, eyeOffsetY + eyeRadius * 0.35 + pupilY);
                    ...
                    ctx.beginPath(); ctx.arc(-eyeOffsetX + pupilX, eyeOffsetY + pupilY, pupilRadius, 0, Math.PI * 2); ctx.fill();
                    ctx.beginPath(); ctx.arc( eyeOffsetX + pupilX, eyeOffsetY + pupilY, pupilRadius, 0, Math.PI * 2); ctx.fill();
                }
                ...
                if (isSpecial) {
                    ctx.strokeStyle = '#0b1020';
                    ctx.lineWidth = eyeRadius * 0.7;
                    const browOffsetY = eyeOffsetY - eyeRadius * 1.25;
                    ctx.beginPath();
                    ctx.moveTo(-eyeOffsetX - eyeRadius * 0.7, browOffsetY + eyeRadius * 0.15);
                    ctx.lineTo(-eyeOffsetX + eyeRadius * 0.7, browOffsetY - eyeRadius * 0.35);
                    ctx.stroke();
                    ctx.beginPath();
                    ctx.moveTo( eyeOffsetX - eyeRadius * 0.7, browOffsetY - eyeRadius * 0.35);
                    ctx.lineTo( eyeOffsetX + eyeRadius * 0.7, browOffsetY + eyeRadius * 0.15);
                    ctx.stroke();
                } else if (isFighting) {
                    ctx.strokeStyle = '#111827';
                    ctx.lineWidth = eyeRadius * 0.5;
                    const browOffsetY = eyeOffsetY - eyeRadius * 1.2;
                    ctx.beginPath();
                    ctx.moveTo(-eyeOffsetX - eyeRadius * 0.6, browOffsetY + eyeRadius * 0.2);
                    ctx.lineTo(-eyeOffsetX + eyeRadius * 0.6, browOffsetY - eyeRadius * 0.2);
                    ctx.stroke();
                    ctx.beginPath();
                    ctx.moveTo( eyeOffsetX - eyeRadius * 0.6, browOffsetY - eyeRadius * 0.2);
                    ctx.lineTo( eyeOffsetX + eyeRadius * 0.6, browOffsetY + eyeRadius * 0.2);
