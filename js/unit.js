...
export class Unit {
    ...
    draw(ctx, isOutlineEnabled, outlineWidth) {
        ...
        // Eyes: simple, large, and covers the whole face
        {
            const headRadius = GRID_SIZE / 2.5;
            const faceWidth = headRadius * 1.8; // near full
            const faceHeight = headRadius * 1.6;
            ...
            if (isDead) {
                ...
                // Eye whites as two big rectangles with bold outline
                const leftX = -faceWidth / 2;
                const rightX = gap / 2;
                const topY = -eyeHeight / 2;
                ctx.fillStyle = isFighting ? 'rgba(255, 200, 200, 1)' : '#ffffff';
                ctx.strokeStyle = '#0f172a';
                ctx.lineWidth = headRadius * 0.25;

                // left eye white
                ctx.beginPath(); ctx.rect(leftX, topY, eyeWidth, eyeHeight); ctx.fill(); ctx.stroke();
                // right eye white
                ctx.beginPath(); ctx.rect(rightX, topY, eyeWidth, eyeHeight); ctx.fill(); ctx.stroke();
                ...
                const maxOffX = eyeWidth * 0.25;
                const maxOffY = eyeHeight * 0.25;
                const offX = Math.cos(ang) * maxOffX;
                const offY = Math.sin(ang) * maxOffY;

                // pupils: rectangles; during fight they are slimmer
                ctx.fillStyle = '#0b1020';
                const basePW = isFighting ? eyeWidth * 0.18 : eyeWidth * 0.28;
                const basePH = isFighting ? eyeHeight * 0.6 : eyeHeight * 0.45;

                // left pupil
                ctx.beginPath();
                ctx.rect(leftX + eyeWidth / 2 - basePW / 2 + offX * 0.6, topY + eyeHeight / 2 - basePH / 2 + offY * 0.6, basePW, basePH);
                ctx.fill();
                // right pupil
                ctx.beginPath();
                ctx.rect(rightX + eyeWidth / 2 - basePW / 2 + offX * 0.6, topY + eyeHeight / 2 - basePH / 2 + offY * 0.6, basePW, basePH);
                ctx.fill();
            }
            ctx.restore();
        }
