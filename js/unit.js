...
export class Unit {
    ...
    update(enemies, weapons, projectiles) {
        ...
        if (this.vampiricStateTimer > 0) {
            ...
            // 흡혈 상태에서는 1초마다 광역 공격
            if (this.vampiricStateTimer > 0 && Math.floor(this.vampiricStateTimer) % 120 === 0) {
                this.attackAnimationTimer = 24; // 낫을 휘두르는 애니메이션 발동 (0.4초)
                this.attackCooldown = 120; // 기본 공격 게이지도 2초로 설정

                // 시각/음향 효과 및 공격 범위 표시를 위한 Effect 생성
                const slashEffect = new VampiricSlashEffect(this.gameManager, this.pixelX, this.pixelY, this, this.facingAngle);
                ...
                const targets = enemies
                    .filter(enemy => Math.hypot(this.pixelX - enemy.pixelX, this.pixelY - enemy.pixelY) < slashEffect.radius && Math.abs(Math.atan2(enemy.pixelY - this.pixelY, enemy.pixelX - this.pixelX) - this.facingAngle) < Math.PI / 2)
                    .sort((a, b) => Math.hypot(this.pixelX - a.pixelX, this.pixelY - a.pixelY) - Math.hypot(this.pixelX - b.pixelX, this.pixelY - b.pixelY))
                    .slice(0, 3);

                targets.forEach(target => {
                    const damage = this.attackPower; // 유닛의 현재 공격력 사용
        ...
        switch (this.state) {
            ...
            case 'AGGRESSIVE':
                if (this.target) {
                    ...
                    if (this.weapon?.type === 'vampiric_scythe' && this.isSpecialAttackReady && this.vampiricStateTimer <= 0) {
                        const distanceToTarget = Math.hypot(this.pixelX - this.target.pixelX, this.pixelY - this.target.pixelY);
                        if (distanceToTarget <= this.detectionRange && gameManager.hasLineOfSight(this, this.target)) {
                            this.weapon.use(this, this.target, true); // 특수 공격 발동
