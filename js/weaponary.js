...
export class VampiricScythe extends Weapon {
    ...
    drawEquipped(ctx, unit) {
        ...
        if (unit.attackAnimationTimer > 0 && unit.vampiricStateTimer > 0) {
            ...
            rotation = -Math.PI / 2 - Math.PI * 0.7 + swingProgress * Math.PI * 1.4;
            xOffset = 10; // 휘두를 때 몸에서 약간 더 멀리
        }

        ctx.rotate(rotation);
        ctx.translate(xOffset, yOffset);
        this.drawScythe(ctx, unit.vampiricStateTimer > 0 ? '#e74c3c' : '#bdc3c7', 0); // 헬퍼 함수에는 추가 회전 없음
        ctx.restore();
    }
...
export class VampiricSlashEffect extends AreaEffect {
    ...
    draw(ctx) {
        ...
        ctx.rotate(this.owner.facingAngle);
