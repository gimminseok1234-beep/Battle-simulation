...
export class VampiricScythe extends Weapon {
    ...
    drawEquipped(ctx, unit) {
        // 유닛이 장착했을 때의 모습
        // [수정] GameManager.render.js에서 회전을 처리하므로 여기서는 회전 로직 제거

        // 유닛의 오른쪽에 위치하도록 좌표 조정
        const orbitRadius = GRID_SIZE * 0.6;
        // [수정] 회전값을 unit.facingAngle로 고정하여 올바른 위치에 있도록 함
        ctx.translate(Math.cos(unit.facingAngle + Math.PI / 2) * orbitRadius, Math.sin(unit.facingAngle + Math.PI / 2) * orbitRadius);

        // [수정] 회전은 render.js에서 처리하므로, 여기서는 기본 각도로 그림
        this.drawScythe(ctx, unit.vampiricStateTimer > 0 ? '#e74c3c' : '#bdc3c7');
    }
