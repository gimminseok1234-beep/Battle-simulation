...
export class Weapon {
    ...
    drawEquipped(ctx, unit) {
        ...
        const use3D = gameManager.is3DStyleEnabled;
        const facing = unit.facingAngle;
        let rotation = facing;
        ...
        // simple hands anchors in local space (when 3D enabled)
        const armSwing = Math.sin(gameManager.animationFrameCounter * 0.15 + (unit.attackCooldown > 0 ? 2 : 0)) * GRID_SIZE * 0.08;
        const rightHandLocal = { x: GRID_SIZE * 0.35, y: -GRID_SIZE * 0.05 - armSwing };
        const leftHandLocal  = { x: -GRID_SIZE * 0.35, y: -GRID_SIZE * 0.05 + armSwing };
        const cos = Math.cos(facing), sin = Math.sin(facing);
        const toWorld = (p) => ({ x: p.x * cos - p.y * sin, y: p.x * sin + p.y * cos });
        const rightW = toWorld(rightHandLocal);
        const leftW = toWorld(leftHandLocal);

        if (!use3D && this.type !== 'lightning' && this.type !== 'ice_diamond') {
            ctx.rotate(rotation);
        }
        ...
        if (this.type === 'sword') {
            ...
            if (use3D) { ctx.translate(rightW.x, rightW.y); ctx.rotate(rotation + Math.PI/6); ctx.scale(0.7, 0.7); }
            else { ctx.translate(-GRID_SIZE * 0.4, 0); ctx.scale(0.7, 0.7); ctx.rotate(-Math.PI / 8); }
            drawMagicDaggerIcon(ctx);
        } else if (this.type === 'axe') {
            if (use3D) { ctx.translate(rightW.x, rightW.y); ctx.rotate(rotation + Math.PI/4); ctx.scale(0.8, 0.8); }
            else { ctx.translate(GRID_SIZE * 0.8, -GRID_SIZE * 0.7); ctx.rotate(Math.PI / 4); ctx.scale(0.8, 0.8); }
            drawAxeIcon(ctx);
        } else if (this.type === 'bow') {
            if (use3D) {
                // draw bow between hands, string pulled to right hand
                const bowDir = Math.atan2(rightW.y - leftW.y, rightW.x - leftW.x);
                ctx.translate(leftW.x, leftW.y);
                ctx.rotate(bowDir);
                const bowScale = 0.7; ctx.scale(bowScale, bowScale);
                ...
                ctx.strokeStyle = '#e5e7eb'; ctx.lineWidth = 2 / bowScale; ctx.beginPath();
                ctx.moveTo(0, -GRID_SIZE*0.8); ctx.lineTo((rightW.x - leftW.x)/bowScale, (rightW.y - leftW.y)/bowScale); ctx.lineTo(0, GRID_SIZE*0.8); ctx.stroke();
            } else {
                ctx.translate(GRID_SIZE * 0.4, 0); ctx.rotate(-Math.PI / 4);
                const bowScale = 0.8; ctx.scale(bowScale, bowScale);
                ctx.fillStyle = '#f3f4f6'; ctx.fillRect(-GRID_SIZE * 0.7, -1, GRID_SIZE * 1.2, 2); ctx.strokeRect(-GRID_SIZE * 0.7, -1, GRID_SIZE * 1.2, 2);
                ...
                ctx.strokeStyle = '#854d0e'; ctx.lineWidth = 4 / bowScale; ctx.beginPath(); ctx.arc(0, 0, GRID_SIZE * 0.8, -Math.PI/2.2, Math.PI/2.2); ctx.stroke();
                ctx.strokeStyle = '#e5e7eb'; ctx.lineWidth = 1.5 / bowScale; ctx.beginPath(); const arcRadius = GRID_SIZE * 0.8, arcAngle = Math.PI / 2.2; ctx.moveTo(Math.cos(-arcAngle) * arcRadius, Math.sin(-arcAngle) * arcRadius); ctx.lineTo(-GRID_SIZE * 0.4, 0); ctx.lineTo(Math.cos(arcAngle) * arcRadius, Math.sin(arcAngle) * arcRadius); ctx.stroke();
            }
        } else if (this.type === 'dual_swords') {
            const drawEquippedCurvedSword = (isRightHand) => {
                ctx.save();
                if (use3D) { const p = isRightHand ? rightW : leftW; const r = isRightHand ? rotation + Math.PI/12 : rotation - Math.PI/12; ctx.translate(p.x, p.y); ctx.rotate(r); }
                else { const yOffset = isRightHand ? GRID_SIZE * 0.6 : -GRID_SIZE * 0.6; const swordRotation = isRightHand ? Math.PI / 8 : -Math.PI / 8; ctx.translate(GRID_SIZE * 0.1, yOffset); ctx.rotate(swordRotation); }
                ctx.fillStyle = '#374151';
            ...
            if (use3D) { ctx.translate(rightW.x, rightW.y); ctx.rotate(rotation + Math.PI/6); this.drawStaff(ctx, 0.8); }
            else { this.drawStaff(ctx, 0.8); }
        } else if (this.type === 'lightning') {
            if (use3D) {
                const revolutionAngle = gameManager.animationFrameCounter * 0.05; const orbitRadius = GRID_SIZE * 0.6; const weaponX = rightW.x + Math.cos(revolutionAngle) * orbitRadius; const weaponY = rightW.y + Math.sin(revolutionAngle) * orbitRadius; ctx.save(); ctx.translate(weaponX, weaponY); this.drawLightning(ctx, 0.48, 0); ctx.restore();
            } else {
                const revolutionAngle = gameManager.animationFrameCounter * 0.05; const orbitRadius = GRID_SIZE * 0.8; const weaponX = Math.cos(revolutionAngle) * orbitRadius; const weaponY = Math.sin(revolutionAngle) * orbitRadius; ctx.save(); ctx.translate(weaponX, weaponY); this.drawLightning(ctx, 0.48, 0); ctx.restore();
            }
        } else if (this.type === 'ice_diamond') {
            if (use3D) { ctx.translate(rightW.x, rightW.y); } else { ctx.translate(GRID_SIZE * 0.6, 0); }
            ...
            if (use3D) { ctx.translate(rightW.x, rightW.y); this.drawMagicSpear(ctx, 0.5, rotation + Math.PI*0.8); }
            else { ctx.translate(GRID_SIZE * 0.2, GRID_SIZE * 0.4); this.drawMagicSpear(ctx, 0.5, -Math.PI / 8 + Math.PI); }
        } else if (this.type === 'boomerang') {
            if (use3D) { ctx.translate(rightW.x, rightW.y); this.drawBoomerang(ctx, 0.5, rotation); }
            else { ctx.translate(0, -GRID_SIZE * 0.5); this.drawBoomerang(ctx, 0.5); }
        } else if (this.type === 'poison_potion') {
            if (use3D) { ctx.translate(rightW.x, rightW.y); this.drawPoisonPotion(ctx, 0.3); }
            else { ctx.translate(0, -GRID_SIZE * 0.5); this.drawPoisonPotion(ctx, 0.3); }
        } else if (this.type === 'hadoken') {
            if (use3D) { ctx.translate(rightW.x, rightW.y); } else { ctx.translate(GRID_SIZE * 0.5, 0); }
            ...
            if (use3D) { ctx.translate(rightW.x, rightW.y); }
            else { ctx.translate(GRID_SIZE * 0.4, GRID_SIZE * 0.3); }
