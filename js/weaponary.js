    ...
    use(unit, target) {
        ...
        if (this.type === 'sword') {
            ...
            if (unit.attackCount >= 3) {
                ...
                // [수정] 3타 특수 공격 시, 'empowered_arrow'만 발사하도록 수정
                gameManager.createProjectile(unit, target, 'empowered_arrow');
                setTimeout(() => {
                    if (unit.hp > 0) {
                        gameManager.createProjectile(unit, target, 'arrow');
                        // [수정] 두 번째 화살도 'empowered_arrow'로 발사
                        gameManager.createProjectile(unit, target, 'empowered_arrow');
                    }
                }, 150);
    ...
    draw(ctx) {
        ...
        if (this.type === 'arrow' || this.type === 'empowered_arrow') { // 일반 화살 및 강화 화살
            ...
            if (this.type === 'empowered_arrow') {
                ctx.shadowColor = '#fef08a'; // 노란색 빛
                ctx.shadowBlur = 10;
                ctx.fillStyle = '#fef9c3'; // 밝은 노란색
            }
            ...
            // [추가] empowered_arrow 이펙트
            if (this.type === 'empowered_arrow' && this.gameManager.random() > 0.3) {
