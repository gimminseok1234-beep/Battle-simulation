...
export class Weapon {
     ...
     */
    use(unit, target, isSpecial = false) {
        const gameManager = this.gameManager;
        if (!gameManager) return;

        if (this.type === 'vampiric_scythe' && isSpecial) {
            unit.vampiricStateTimer = 300; // 5초간 흡혈 모드
            gameManager.audioManager.play('powerUp'); // 효과음
            return;
        }
    ...
    draw(ctx) {
        ...
        const scale = (this.type === 'crown') ? 1.0 : (this.type === 'lightning' ? 0.6 : (this.type === 'magic_spear' ? 0.78 : (this.type === 'poison_potion' ? 1.2 : (this.type === 'boomerang' ? 0.49 : 0.8))));
        ctx.save();
     ...
     * @param {Unit} unit 
     */
    drawEquipped(ctx, unit, isGlowEffect = false) {
        const gameManager = this.gameManager;
        if (!gameManager) return;
...
export class VampiricScythe extends Weapon {
    constructor(gameManager, x, y) {
        ...
        this.attackPowerBonus = 10; // 유닛 기본 공격력(5) + 10 = 15
    ...
    drawEquipped(ctx, unit, isGlowEffect = false) {
        ...
        ctx.translate(Math.cos(unit.facingAngle - Math.PI / 2) * orbitRadius, Math.sin(unit.facingAngle - Math.PI / 2) * orbitRadius);

        let rotation = unit.facingAngle + Math.PI / 2; // 기본 각도

        // 광역 공격 애니메이션
        if (unit.attackAnimationTimer > 0 && unit.vampiricStateTimer > 0) {
            const duration = 24;
            const progress = (duration - unit.attackAnimationTimer) / duration; // 0 -> 1
            // Ease-in-out-cubic 함수로 부드러운 가감속 구현
            const swingProgress = progress < 0.5 ? 4 * progress * progress * progress : 1 - Math.pow(-2 * progress + 2, 3) / 2;
            
            // 낫을 뒤로 뺐다가 크게 휘두르는 애니메이션
            rotation = unit.facingAngle + Math.PI / 2 - Math.PI * 0.7 + swingProgress * Math.PI * 1.4;
        }

        ctx.rotate(rotation);

        // 빛나는 효과를 그릴 때는 실제 낫 모양을 그리지 않음
        if (!isGlowEffect) {
            this.drawScythe(ctx, unit.vampiricStateTimer > 0 ? '#e74c3c' : '#bdc3c7', 0);
        }
...
export class VampiricSlashEffect extends AreaEffect {
    constructor(gameManager, x, y, owner, angle) {
        super(gameManager, x, y, 'vampiric_slash', {
            damage: 15, // 광역 공격 데미지
            ownerTeam: owner.team,
            radius: GRID_SIZE * 4,
            duration: 20, // 이펙트 지속 시간
        });
        this.owner = owner;
        ...
        // 화려한 빨간색 이펙트 생성
        for (let i = 0; i < 30; i++) {
            const angle = this.angle + (Math.random() - 0.5) * Math.PI * 1.5;
        ...
        this.gameManager.audioManager.play('slash');
    }

    update() {
        this.duration -= this.gameManager.gameSpeed;

        // 궤적에 있는 적을 최대 3명까지 찾아 공격 및 흡혈
        const enemies = this.gameManager.units.filter(u => u.team !== this.ownerTeam && u.hp > 0 && !this.hitTargets.has(u));
        const targetsToHit = enemies
            .filter(enemy => {
                const dist = Math.hypot(this.owner.pixelX - enemy.pixelX, this.owner.pixelY - enemy.pixelY);
                const angleToEnemy = Math.atan2(enemy.pixelY - this.owner.pixelY, enemy.pixelX - this.owner.pixelX);
                const angleDiff = Math.abs(angleToEnemy - this.angle);
                return dist < this.radius && (angleDiff < Math.PI / 2 || angleDiff > Math.PI * 1.5);
            })
            .slice(0, 3 - this.hitTargets.size);

        targetsToHit.forEach(target => {
            const damage = this.owner.attackPower;
    ...
    draw(ctx) {
        ...
        const animationDuration = 24; // unit.attackAnimationTimer와 동일하게 설정
        const effectDuration = 20;
        const progress = 1 - (this.duration / effectDuration);
        const opacity = Math.sin(progress * Math.PI); // 나타났다가 사라지는 효과

        // 낫 애니메이션 진행률에 맞춰 궤적을 그림
        const swingProgress = Math.min(1, (animationDuration - this.owner.attackAnimationTimer) / animationDuration);

        ctx.save();
        ctx.translate(this.owner.pixelX, this.owner.pixelY);
        ctx.rotate(this.angle);
        ctx.strokeStyle = `rgba(231, 76, 60, ${opacity * 0.8})`;
        ctx.lineWidth = 4;
        ctx.beginPath();
        ctx.arc(0, 0, this.radius, -Math.PI / 2, -Math.PI / 2 + (Math.PI * swingProgress));
        ctx.stroke();
        ctx.restore();
    }
}
