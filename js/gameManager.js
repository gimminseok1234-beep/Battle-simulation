...
import { createToolboxUI as buildToolboxUI, showHomeScreen as uiShowHomeScreen, showDefaultMapsScreen as uiShowDefaultMapsScreen, showReplayScreen as uiShowReplayScreen, showEditorScreen as uiShowEditorScreen } from './ui.js';
...
export class GameManager {
    ...
    showHomeScreen() {
        uiShowHomeScreen(this);
    }

    showDefaultMapsScreen() {
        uiShowDefaultMapsScreen(this);
    }

    showReplayScreen() {
        uiShowReplayScreen(this);
    }

    async showEditorScreen(mapId) {
        await uiShowEditorScreen(this, mapId);
    }
    ...
    setupEventListeners() {
        ...
        const style3DToggle = document.getElementById('style3DToggle');
        if (style3DToggle) {
            style3DToggle.checked = this.is3DStyleEnabled;
            style3DToggle.addEventListener('change', (e) => {
                this.is3DStyleEnabled = e.target.checked;
                this.draw();
            });
        }
    ...
    drawMap() {
        for (let y = 0; y < this.ROWS; y++) {
            for (let x = 0; x < this.COLS; x++) {
                ...
                const px = x * GRID_SIZE, py = y * GRID_SIZE;
                if (!this.is3DStyleEnabled || (tile.type !== TILE.WALL && tile.type !== TILE.CRACKED_WALL && tile.type !== TILE.GLASS_WALL)) {
                    this.ctx.fillRect(px, py, GRID_SIZE, GRID_SIZE);
                } else {
                    // Base wall body
                    this.ctx.fillRect(px, py, GRID_SIZE, GRID_SIZE);
                    // Top face
                    const topHeight = Math.floor(GRID_SIZE * 0.35);
                    this.ctx.fillStyle = this.shadeColor(this.ctx.fillStyle, 25);
                    this.ctx.fillRect(px, py - topHeight, GRID_SIZE, topHeight);
                    // Right side face shadow
                    this.ctx.fillStyle = this.shadeColor(this.ctx.fillStyle, -25);
                    this.ctx.beginPath();
                    this.ctx.moveTo(px + GRID_SIZE, py);
                    this.ctx.lineTo(px + GRID_SIZE, py - topHeight);
                    this.ctx.lineTo(px + GRID_SIZE - 4, py - topHeight + 4);
                    this.ctx.lineTo(px + GRID_SIZE - 4, py + 4);
                    this.ctx.closePath();
                    this.ctx.fill();
                }
